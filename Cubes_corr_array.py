...
def generate(galaxyname='M51',vmin=40, vmax=80, ymin=200, ymax=400, xmin=360, xmax=560, deltaX=40, deltaV=3, deltadeltaX=1, deltadeltaV=1, nmax=201, xithreshold=0.7):
	...
	Takes a xi surface map whose name matches the given parameters, finds the angle
		...
		returns a 1D array containing the (interpolated) values of xi along this
		line.
	...
	nmax : int
		...
		Higher is better.
	xithreshold : float
		This is the threshold percentage of xi along the principal
		axis for which coordinates will be returned.
	...
	theta : float
		...
		origin has minimal xi.
	linearray1_min : array (1D, float)	
		A list of minimal xi values along the principal
	...
	thres_radii : array (1D, float)
		...
		xi is greater than the xithreshold percentage, 
...
#	if normalization==False:							# Ensures that the code doesn't return a functional value for the xi/xi.max() > xithreshold list 
#		thres_radii = np.nan							#	if the xi map is not normalized.
...
def slicer(theta, array, nmax=201):
	"""
	Takes a xi surface map, cuts a thin "slice" through the middle at angle 
	...
	array : array
		The xi map that we're dealing with.
	nmax (int)
	...
	linearrayx : array (1D, float)
		...
		is the maximum radius of the xi surface map.
	linearray1 : array (1D, float)	
		A list of xi values along the principal axis
		(i.e. the line of minimal xi), with the 
	...
	maxradius1 : float
		...
		axis from the center of the xi surface map
...
def plot(theta,maxradius1,maxradius2,array,linearrayx,linearray1,linearray2, filename="paws_norot", imagename="defaultname", deltaX=40, deltaV=3, xithreshold=0.7):
	'''
	Given a map, its position angle, the values along
		the low-xi "slice", and other information,
	...
	-----------
	theta : float
		Angle of minimal xi along the provided map,
	...
	linearrayx : array (1D, float)
		...
		Found using "Cubes_array.slicer".
	linearray1 : array (1D, float)
		A list of xi values along the line of
		minimal xi, with the origin in the middle 
		of the list.
		Found using "Cubes_array.slicer".
	linearray2 : array (1D, float)
		A list of xi values perpendicular to the line of
		minimal xi, with the origin in the middle 
		of the list.
		Found using "Cubes_array.slicer".
	filename : string
	...
	deltaX, deltaV : int
		Parameters used in the relevant xi map.
	xithreshold : float
		This is the threshold percentage of xi along the principal
		axis for which coordinates will be returned.
	...
	-----------
	linearray1_min : array (1D, float)	
		A list of local minima of xi along the principal axis, 
		with the origin in the middle of the list.
	thres_radii : array (1D, float)
		A list of radii along the principal axis for which
		xi is greater than xithreshold, with the origin in
	...
	ax2.set_title('xi along "Line of Lowest xi", versus Radius')
	ax2.set_xlabel('Distance from Center along Line (pc)')
	ax2.set_ylabel('xi')
	ax2.legend(loc='lower left')

	plt.tight_layout()
	plt.savefig("xi_minimal_"+imagename+".png")
	plt.clf()
	# -------------------
