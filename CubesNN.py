...
# 6.13.16 - Working with Subcubes from rotation-corrected .fits files, in spatial and spectral dimensions. Removes noise by default!
...
def cubegen(vmin,vmax,ymin,ymax,xmin,xmax, filename = "paws_norot", drawmap = False, mapname="3Dcube"):
	...
	subcube = np.copy( data[vmin:vmax,ymin:ymax,xmin:xmax].value )
	subcube[ subcube<Tmin ] = np.nan

	if drawmap == True:
		plt.figure(1)

		plt.imshow(np.nanmax(subcube,axis=0), extent=[(xmin-xshape)*pixelwidthPC,(xmax-xshape)*pixelwidthPC, \
		...
		plt.savefig('galaxy_NN_'+mapname+'.png')
...
def structgen(subcube0, deltaX=30, deltaV=3, deltadeltaX=1, deltadeltaV = 1, normalization=False):
	...
	for delv in range (0,dV/ddV+1):                 # "delv" defined as "dv / ddV"
		...
		for dely in range (-dY/ddY,dY/ddY+1):   # "dely" defined as "dy / ddY"
		    ...
		    M = data0	         		# This is the subcube information (unitless).
...
def mapgen(S_2, deltaX=30, deltaV=3, deltadeltaV=1, mapname="3Dcube", filename = "paws_norot"):
	...
	plt.colorbar()
	plt.savefig('map_NN_'+mapname+'.png')
...
#	plt.ylim([0.9,1.1])
	...
	plt.savefig('plot_NN_'+mapname+'.png')
	plt.clf()
...
def everythinggen(vmin, vmax, ymin, ymax, xmin, xmax, S_2, deltaX, deltaV, deltadeltaX, deltadeltaV, imagename, filename):
	...
	subcube = data[vmin:vmax,ymin:ymax,xmin:xmax].value
	subcube[ subcube<0.5 ] = np.nan
	...
	### Surface
	ax2.imshow(S_2[0], interpolation = 'none', extent = [-dX*pixelwidthPC,dX*pixelwidthPC,-dY*pixelwidthPC,dY*pixelwidthPC], \
		   vmin=0, vmax=S_2.max(), aspect='auto', origin='lower')
	...
	### /Plot
	plt.tight_layout()
	plt.savefig('entire_NN_'+imagename+'.png')
	plt.clf()
