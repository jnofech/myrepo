...
def S2_array(vmin, vmax, ymin, ymax, xmin, xmax, deltaX = 100, deltaV = 3, deltadeltaX = 10, deltadeltaV = 1, filename="paws_norot", drawmap=False, galaxyname='M51'):
	"""
	   ...
	   specified dimensions; using the .fits file in "CubesNN.py".
	...
	if deltadeltaX == 1 and deltadeltaV == 1:
		tempname = 'saved_S2array_NN_'+imagename+'_dV_is_'+str(deltaV)+'_dX_is_'+str(deltaX)+'_MAXRES'
	else:
		tempname = 'saved_S2array_NN_'+imagename+'_dV_is_'+str(deltaV)+'_dX_is_'+str(deltaX)

	subcube = CubesNN.cubegen(vmin,vmax,ymin,ymax,xmin,xmax,filename,drawmap, imagename)	# Will draw a map of the subcube if drawmap=True.

	S_2 = CubesNN.structgen(subcube,deltaX,deltaV,deltadeltaX,deltadeltaV,False)
...
def S2_draw(vmin, vmax, ymin, ymax, xmin, xmax, deltaX = 100, deltaV = 3, deltadeltaX = 10, deltadeltaV = 1, filename="paws_norot", galaxyname='M51'):
	...
	CubesNN.mapgen(S_2, deltaX, deltaV, deltadeltaV, imagename, filename)
	CubesNN.plotgen(S_2, deltaX, deltaV, deltadeltaX, deltadeltaV, imagename, filename)
	CubesNN.everythinggen(vmin, vmax, ymin, ymax, xmin, xmax, S_2, deltaX, deltaV, deltadeltaX, deltadeltaV, imagename, filename)
...
def S2_arrayM51(vmin=40,vmax=80, deltaX=40, deltaV=3, deltadeltaX=10, deltadeltaV=1, drawmap = False):
	...
	if drawmap == True:
		...
		plt.imshow(np.nanmax(map[vmin:vmax],axis=0), vmin=0, extent=[-xshape*pixelwidthPC,xshape*pixelwidthPC,-yshape*pixelwidthPC,yshape*pixelwidthPC], origin='lower')
		plt.colorbar()
		fig = plt.gcf()
		...
		plt.savefig('galaxy_NN_'+galaxyname+'_'+str(vmin)+'to'+str(vmax)+'_entire.png')
		plt.clf()
		...
		yshape = data.shape[1]/2.0
		xshape = data.shape[2]/2.0
		plt.imshow(np.nanmax(map[vmin:vmax],axis=0), vmin=0, origin='lower')
		...
		plt.colorbar()
		plt.savefig('galaxy_NN_'+galaxyname+'_'+str(vmin)+'to'+str(vmax)+'_regions.png')
		plt.clf()
...
def compare_S2array(vmin=40,vmax=80, deltaX=40, deltadeltaX=10):
	...
	drawmap = False
	if deltadeltaX == 1:
		savename = 'savedfile_NN_M51andM33_comparison_MAXRES'
	else:
		savename = 'savedfile_NN_M51andM33_comparison'
	...
	subcubeM51 = CubesNN.cubegen(vmin,vmax,yminM51,ymaxM51,xminM51,xmaxM51, filenameM51, drawmap)	# Subcube for M51.
	subcubeM51_n = CubesNN.cubegen(0,20,yminM51,ymaxM51,xminM51,xmaxM51, filenameM51, drawmap)	# Noise subcube for M51.

	subcubeM33 = CubesNN.cubegen(vmin,vmax,yminM33,ymaxM33,xminM33,xmaxM33, filenameM33, drawmap)	# Subcube for M33.
	subcubeM33_n = CubesNN.cubegen(0,20,yminM33,ymaxM33,xminM33,xmaxM33, filenameM33, drawmap)	# Noise subcube for M33.

	S2_M51 = CubesNN.structgen(subcubeM51,deltaX,0,deltadeltaX,1,False) - CubesNN.structgen(subcubeM51_n,deltaX,0,deltadeltaX,1,False)
	S2_M33 = CubesNN.structgen(subcubeM33,deltaX,0,deltadeltaX,1,False) - CubesNN.structgen(subcubeM33_n,deltaX,0,deltadeltaX,1,False)
...
def compare_S2draw(vmin=40,vmax=80, deltaX=40, deltadeltaX=10):
	...
	plt.savefig('plot_NN_M51andM33_NN.png')
	plt.clf()
